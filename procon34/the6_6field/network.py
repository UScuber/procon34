...
class SimpleCNN(tf.keras.Model):
    ...
    def __init__(self, action_space, filters=512, use_bias=False):
        ...
        self.conv1 = kl.Conv2D(filters, 3, padding='same', use_bias=use_bias)#kernel_size=3
        ...
        self.pi = kl.Dense(self.action_space, activation='softmax')#出力はself.action_space次元
...
if __name__ == "__main__":
    ...
    x = encampment.encode_state(state, current_player=1)
    x = x[np.newaxis, ...]
    print(x.shape)
    action_space = encampment.N_ROWS * encampment.N_COLS
