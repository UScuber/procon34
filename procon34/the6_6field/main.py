...
# rayの扱えるremote関数にする。使用するリソースを決める
@ray.remote(num_cpus=1, num_gpus=0)
def selfplay(weights, num_mcts_simulations, dirichlet_alpha):

    record = []

    state = encampment.get_initial_state()

    network = AlphaZeroResNet(action_space=encampment.ACTION_SPACE)

    network.predict(encampment.encode_state(state, 1))
    ...
    while not done:
        ...
        if i <= 10:
            ...
            action = np.random.choice(
                range(encampment.ACTION_SPACE), p=mcts_policy)
        else:
        ...
        next_state, done = encampment.step(state, action, current_player)
    ...
    #: win: 1, lose: -1, draw: 0
    reward_first, reward_second = encampment.get_result(state)
...
def testplay(current_weights, num_mcts_simulations,
    ...
    dummy_state = encampment.get_initial_state()

    network.predict(encampment.encode_state(dummy_state, 1))
    ...
    for n in range(n_testplay):
        ...
        while not done:
            ...
            if current_player == alphazero:
                ...
                action = encampment.greedy_action(state, current_player, epsilon=0.3)
        ...
        stone_first, stone_second = encampment.count_stone(state)
        ...
        encampment.save_img(state, "img", f"test_{n}.png", message)
...
def main(num_cpus, n_episodes=10000, buffer_size=40000,
    ...
    #使用するcpuの数とgpuの数を設定する　
    #: 開発・デバック時はlocal_mode=Trueで起動すると楽(サブプロセス化されないのでprint文が動く)
    ...
    dummy_state = encampment.encode_state(encampment.get_initial_state(), 1)
    ...
    while n <= n_episodes:
        ...
        for _ in tqdm(range(update_period)):
            ...
            # ray.wait()
            # 第一引数は並列処理を行うやつ
            # 第二引数はすでに終了しているタスクが第二引数個格納されたリストとそれ以外のタスクが格納されたリストを返す。
